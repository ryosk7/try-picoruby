...
class PWMChannel {
    constructor(pwm, clock, index) {
        ...
        this.clock = clock;
        this.index = index;
        this.timer = new timer32_js_1.Timer32(this.clock, this.pwm.clockFreq);
        this.alarmA = new timer32_js_1.Timer32PeriodicAlarm(this.timer, () => {
            this.setA(false);
        });
        this.alarmB = new timer32_js_1.Timer32PeriodicAlarm(this.timer, () => {
            this.setB(false);
        });
        this.alarmBottom = new timer32_js_1.Timer32PeriodicAlarm(this.timer, () => this.wrap());
        this.csr = 0;
        this.div = 0;
        this.cc = 0;
        this.top = 0;
    ...
    writeRegister(offset, value) {
        switch (offset) {
            case CHn_CSR:
                ...
                this.timer.mode = value & CSR_PH_CORRECT ? timer32_js_1.TimerMode.ZigZag : timer32_js_1.TimerMode.Increment;
                break;
            case CHn_DIV: {
...
class RPPWM extends peripheral_js_1.BasePeripheral {
    constructor() {
        super(...arguments);
        this.channels = [
    ...
    channelInterrupt(index) {
        ...
        this.rp2040.dma.setDREQ(dma_js_1.DREQChannel.DREQ_PWM_WRAP0 + index);
    }
    checkInterrupts() {
        this.rp2040.setInterrupt(irq_js_1.IRQ.PWM_WRAP, !!this.intStatus);
    }
    gpioSet(index, value) {
        const bit = 1 << index;
